...
In our experiment we will implement ADVI for deep learning models. In our project, neural network consists of several layers of neurons, output $z$ of each neuron depends on previous layer through some linear combination and activation function:
\begin{equation*}
	z = g\big(\sum_{i=1}^{N} w_ix_i + b\big),
\end{equation*}
where $x_i$ is the output from previous layer, $w_i$ -- weight corresponding to this output and $b$ is a bias term. The usual approach in deep learning is to use stochastic gradient descent and error backpropagation in order to estimate the parameters of the network.

We will implement bayesian approach, i.e. instead of point estimates, each parameter and each output in the network is probability distribution, the difference between two approaches can be seen on figure \ref{pic:BNN}. Then, we are able to write the new definition for neurons:
\begin{equation*}
p(z) = g\big(\sum_{i=1}^{N} p(w_i)p(x_i) + p(b)\big).
\end{equation*}
Now we can easily see that our neural network is indeed bayesian model, if we set all the priors for the parameters. After that we can train our network using ADVI instead of backpropagation.
\begin{figure}[h]
	\centering
...
\subsection*{Uncertainty in predictions on real data}

In this experiment we used MNIST datasets (50000/10000 objects for training/validation). The main goal was to use ADVI approach on real data with several classes and analyze the uncertainties in predictions. Here we used a network with the following structure:
...
Using the estimation of posterior distributions of the considered neural network parameters we generated $100$ samples for each object from validation dataset and computed the variances of predictions over all classes and approximated expected error rate, see fig.~\ref{pic: var_eer}.
...
On the next picture (see fig.~\ref{pic: high_conf}) one can see the most interesting cases for the neural network. On these pictures our model made a mistale but the expected error rate was equal to zero. 
The pictures with the lowest confidence were shown on the fig.~\ref{pic: low_conf} .

...
We implemented bayesian approach, i.e. instead of point estimation, each parameter and each output in the network is given by probability distribution, the difference between two approaches can be seen on fig.~\ref{pic:BNN}. This probability distributions correspond to the priors of the bayesian model described above. The usual models tend to overfit in the case of high-dimensional parameter space. This is exactly the case of deep neural networks, where number of estimated parameters in order of millions. There are some techniques to avoid this problem. The most widely used  is regularization. Adding the term which restricts the model complexity allows to control parameters values. The introducing priors distributions is exactly the same procedure. The normal prior corresponds to $\ell 2$ regularization and the laplace prior~--- $\ell 1$ regularizer.
\begin{figure}[h]
	\centering
...
In this experiment our model we used the neural network with the following structure:

$\bullet$ Dense layer with $5$ units of the layer and \textit{tanh} activation;

$\bullet$ Dense layer with $5$ units of the layer and \textit{tanh} activation;

$\bullet$ Dense layer with $2$ units of the layer and \textit{softmax} as activation.
...
In the plots below one could see the same results for moons dataset. Both dataset shows that our model become uncertain about predictions near the true decision boundary.
...
The hyperpriors allows to find the appropriate hyperparameters values for normal and laplace priors. The fixed values was tuned on the logarithmic grid. 
...
$\bullet$ posterior distribution helps to make conclusions about certainties;

$\bullet$ hyperpriors is almost all gives the better results, especially on noisy data.
...
In the fig.~\ref{pic: var_eer} the first plot illustrates the variances of predictions over all samples. One could these variances for each class averaged over misclassified objects (blue line), objects with correct prediction (green line) and all objects (red line). The variances for misclassified objects is much higher than for correct. Also one could see that that for digits 2, 7, 8, 9 the results are more dispersed. The second plot shows the histogram of the expected error rate which expresses the fraction of samples for which the prediction is different from the mode of the posterior distribution.
...
The fig.~\ref{pic: high_conf} demonstrates the digits from validation sets for which the expected error rate equals zero, but the model makes mistake. It means that our model overconfident about it results, but there is a mistake. There are exactly three of them. 
...
The next picture (see fig.~\ref{pic: high_conf}) shows three digits for which the expected error rate is the highest (about 0.7). For these objects our model is not confidence at all. Also we gives the true labels and models predictions for these examples.

The model accuracy for these dataset is $97.7 \%$. We used the different setup for priors and hypepriors. But the final model which examples shown above includes normal prior with fixed values of hyperparameters.
...
The bayesian approach allows to use transfer learning approach to build a model on new object recognition data set using pretrained on another dataset model. In order to implement this idea one could place informed priors centered around weights retrieved from this pretrained network. We tried this approach for two real datasets. The first one is CIFAR-10 dataset \footnote{%
...
\href{ https://aws.amazon.com/}{https://aws.amazon.com/}} on g2.2xlarge. Unfortunately, our approach lack of memory in this setup for both datasets. Then we tried to cut off last layers to reduce number of estimated parameters. When we got rid of all dense layers we coped with compiling the model, but it would took days to fit the model. Since that, we didn't get the results for these datasets.
\begin{figure}
\centering
...
\textbf{Roman Isachenko} extended the bayesian approach to hierarchical modelling, build VGG-19 network with transfer learning approach.

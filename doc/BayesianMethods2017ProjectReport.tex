...
$\bullet$ DenseLayer with $5$ units of the layer and \textit{tanh} as activation function;

$\bullet$ DenseLayer with $5$ units of the layer and \textit{tanh} as activation function;

$\bullet$ DenseLayer with $2$ units of the layer and \textit{softmax}.

In this experiment we try different values of the noise parameter. As expected with high noise parameter it is hard to reconstruct the initial pattern (it is easy to see the form of it on the fig.\ref{pic: ex_data}). In our experiment we considered the hardest datasets. On the picture below one can see the best approach that coped with the task for circles dataset ($700$ samples for training, $300$ for validation):
...
Here one can see the same picture that also demonstrates posterior probability over the fist class and uncertainty in predictions of the best approach but for moons datasets ($700$ samples for training, $300$ for validation):
...
In the next experiment we compare the results of approaches that used fixed values for hyper-parameters and approaches that use hyper-priors. The next pictures demonstrates the posterior probability over the first class for different priors on the circles datasets:
...
In the next experiment we visualize the estimated posterior distributions of the weights from the last layer for different priors: Laplace and Cauchy. As expected on the fig.~\ref{pic: sparsity} we can see that for Laplace prior many of weights are near to zero compared to Cauchy prior. Hence we obtain that Laplace priors is connected to the sparse solution as it should be.
...
\begin{figure}[h]
	...
	\caption{Estimated posterior distributions of the weights from the last layer}
	\label{pic: sparsity}
\end{figure}

Thus, according to the results of the all conducted experiments on the synthetic data we can formulate several \textbf{conclusions:}
...
$\bullet$ hyper-priors is almost all gives the better results, especially on noisy data.

\subsection*{Uncertainty in predictions on real data}

In this experiment we use MNIST datasets (50000 for training ad 10000 for validation). The main goal was to use ADVI approach on real data with several classes and analyze the uncertainties in predictions. Here we used a network with the following structure:

\vspace{\baselineskip}

\begin{minipage}[t]{0.64\columnwidth}
	\vspace{-6cm}
	$\bullet$ Conv2DLayer with $32$ filters of the size $5\times 5$ and \textit{tanh} nonlinearity;
	
	$\bullet$ MaxPool2DLayer where pool size is equal to $2\times2$;
	...
	$\bullet$ DenseLayer with $64$ units of the layer and \textit{tanh} as activation function;
	
	$\bullet$ DenseLayer with $10$ units of the layer and \textit{softmax}.
\end{minipage}
\hspace{0.3cm}

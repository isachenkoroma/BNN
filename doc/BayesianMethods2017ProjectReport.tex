...
We will implement bayesian approach, i.e. instead of point estimates, each parameter and each output in the network is probability distribution, the difference between two approaches can be seen on figure \ref{pic:BNN}.  Now we can easily see that our neural network is indeed bayesian model, if we set all the priors for the parameters. After that we can train our network using ADVI instead of backpropagation.
\begin{figure}[h]
	\centering
